...
class Promotion extends CI_Controller
{
    ...
    function __construct()
    {
        ...
        $this->name = "promotion/promotion/"; 
        $this->dir_path = "promotion/";
    }
    ...
    public function promotion_lists()
    {
        ...
        $data['page']['page_title'] = APP_NAME . LIST_PROMOTION; /* Add rides title*/
        $data['page']['css'] = 'list_css'; /* add rides css */
        $this->load->view($this->dir_path.'promotion_lists',$data);/* load list.php of rides folder for list of rides */
    ...
    {
        if ($_FILES) {
            $image = $this->file_upload($_FILES['image'], PROMOTION_IMG_DIR);
            ...
            $in_array = array(
                    ...
                    $this->db->insert('tbl_promotion', $in_array);
        ...
        {
            $data['page']['page_title'] = APP_NAME . ADD_PROMOTION; // Add rides title
            $data['page']['css'] = 'form_css'; // add rides css 
            $this->load->view($this->dir_path.'promotion_post', $data); //load post.php of rides folder for add of rides 
        }
     ...
     public function promotion_update($id)
    {
        if ($_FILES) {
            ...
            $image_old = $this->db->where(array('id' => $id,'status' => '1'))->get('tbl_promotion')->row();
            $image = $this->file_upload($_FILES['image'], PROMOTION_IMG_DIR);
            $image = ($image) ? $image : $image_old->image;
                    ...
                    // print_r($in_array); exit();
            $this->db->where('id', $id)->update('tbl_promotion', $in_array);
        ...
        else 
        {
            ...
            $result['id'] = $id;
            $data['result'] = $result;
            $data['page']['page_title'] = APP_NAME . EDIT_PROMOTION; /* Add default_room title*/
            $data['page']['css'] = 'form_css'; /* add default_room css */
            $this->load->view($this->dir_path.'promotion_update', $data);/* load update.php of default_room folder for edit of default_room */
        }
    ...
    public function promotion_delete($id){
        ...
        $this->db->update('tbl_promotion', $data);
